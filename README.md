...
3. **EIP 할당**
   ...
   AWS 의 고정 IP를 Elastic IP (EIP, 탄력적 IP)라고 한다.   

   인스턴스도 결국 하나의 서버이기 때문에 IP가 존재한다.  
   인스턴스 생성 시에 항상 새 IP를 할당하는데, 한 가지 조건이 더 있다.   
   같은 인스턴스를 중지하고 다시 시작할 때도 새 IP가 할당된다.   
   즉, 요금을 아끼기 위해 잠깐 인스턴스를 중지하고 다시 시작하면 IP가 변경되는 것이다.   
   이렇게 되면 매번 접속해야하는 IP가 변경돼서 PC에서 접근할 때마다 IP주소를 확인해야한다.
   ...
   **👿 주의**
   ...
   또한, 만약 더는 사용할 인스턴스가 없을 때도 탄력적 IP를 삭제해야한다.

4. **EC2 서버에 접속하기**

5. **아마존 리눅스 1 서버 생성 시 꼭 해야 할 설정들 적용**
...
MySQL, MariaDB, PostgreSQL 중 MariaDB로 구축할 것이다.

**🙋‍♀️ 왜 ?** 
가격 : 무료
Amazon AUrora 교체 용이하다.
(*Amazon AUrora : AWS에서 MySQL과 PostgreSQL을 클라우드 기반에 맞게 재구성한 DB. )

**MariaDB의 MySQL 대비 장점?**
